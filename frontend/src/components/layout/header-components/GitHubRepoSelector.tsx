...
import { useGitHubRepositories, useGitHubRepository } from '../../../features/github/hooks';
...
/**
 * Compact GitHub repository selector for the header
 */
export const GitHubRepoSelector = () => {
  ...
  const { data: repoInfo } = useGitHubRepository(githubOwner ?? undefined, githubRepo ?? undefined);
  ...
  const fullName = repoInfo?.fullName || (githubOwner && githubRepo ? `${githubOwner}/${githubRepo}` : null);
  const htmlUrl = repoInfo?.htmlUrl;
  ...
  useEffect(() => {
    const currentFullName = githubOwner && githubRepo ? `${githubOwner}/${githubRepo}` : undefined;
    if (currentFullName && currentFullName !== prevRepoRef.current && githubOwner && githubRepo) {
  ...
  const handleSubmit = useCallback((value: string) => {
    ...
    if (trimmed.includes('/')) {
      ...
      if (owner && repo) {
        setGitHubRepo(owner.trim(), repo.trim());
        setIsOpen(false);
        setInputValue('');
      }
    }
  ...
  const handleSelectRepo = useCallback((repo: RepositorySummary | RepoConfig) => {
    let owner: string;
    let repoName: string;

    if ('name' in repo) {
      // It's a RepositorySummary
      owner = repo.owner;
      repoName = repo.name;
    } else {
      // It's a RepoConfig
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span className="font-medium truncate max-w-[180px]" title={fullName || 'Select repository'}>
          ...
          <div className="p-2 border-t flex items-center gap-2" style={{ borderColor: 'var(--border)' }}>
            ...
            {htmlUrl && (
              <a
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs transition-colors ml-auto"
                style={{ color: 'var(--text-secondary)' }}
              >
