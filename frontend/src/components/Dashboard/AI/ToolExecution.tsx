...
export function ToolExecution({
  ...
  const getStatusBackground = () => {
    switch (status) {
      case 'pending':
        return theme === 'full-dark' ? 'bg-zinc-800' : 'bg-gray-100 dark:bg-gray-700';
      case 'success':
  ...
  const getParameterBackground = () => {
    if (theme === 'full-dark') {
      return 'bg-zinc-800 hover:bg-zinc-700';
    }
    return 'bg-gray-50/50 dark:bg-gray-800/50 hover:bg-gray-100/50 dark:hover:bg-gray-700/50';
  };
  ...
  return (
    <motion.div
      ...
      className={`rounded-lg border ${getToolBorder()} overflow-hidden
        backdrop-blur-md shadow-sm hover:shadow-md transition-all duration-200`}
    >
      {/* Tool Header */}
      <div className={`px-4 py-3 ${getToolHeaderBackground()} 
        border-b ${getToolBorder()}`}>
      ...
      <div className="p-4">
        {status === 'pending' ? (
          ...
          <div className="prose prose-sm dark:prose-invert">
            <pre className={`text-sm ${getPreBackground()} rounded-md p-3 overflow-x-auto`}>
              {result}
            </pre>
          </div>
        ) : error ? (
        ...
        {tool.parameters && Object.keys(tool.parameters).length > 0 && (
          <div className={`mt-3 pt-3 border-t ${getToolBorder()}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {tool.required_permissions.map((permission) => (
                <span key={permission} className={`inline-flex items-center px-2.5 py-0.5 
                  rounded-full text-xs font-medium ${theme === 'full-dark' ? 'bg-zinc-800 text-zinc-200' : 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200'}`}>
                  {permission}
                </span>
              ))}
            </div>
