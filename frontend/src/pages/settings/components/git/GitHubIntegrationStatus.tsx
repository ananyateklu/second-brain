/**
 * GitHub Integration Status Card
 * Shows the current GitHub integration configuration status
 */

import { useGitHubIntegrationStatus } from '../../hooks/use-git-github-status';

export function GitHubIntegrationStatus() {
  const { data: status, isLoading, error } = useGitHubIntegrationStatus();

  const getStatusColor = () => {
    if (isLoading) {
      return {
        bg: 'var(--color-gray-400)',
        shadow: 'color-mix(in srgb, var(--color-gray-400) 20%, transparent)',
      };
    }
    if (error || !status?.isConfigured) {
      return {
        bg: 'var(--color-warning)',
        shadow: 'color-mix(in srgb, var(--color-warning) 20%, transparent)',
      };
    }
    if (!status.hasDefaultRepository) {
      return {
        bg: 'var(--color-brand-600)',
        shadow: 'color-mix(in srgb, var(--color-brand-600) 20%, transparent)',
      };
    }
    return {
      bg: 'var(--color-success)',
      shadow: 'color-mix(in srgb, var(--color-success) 20%, transparent)',
    };
  };

  const statusColor = getStatusColor();

  return (
    <div
      className="rounded-2xl border p-4 transition-all duration-200"
      style={{
        backgroundColor: 'color-mix(in srgb, var(--text-primary) 3%, transparent)',
        borderColor: 'color-mix(in srgb, var(--text-primary) 6%, transparent)',
      }}
    >
      <div className="flex items-center gap-3">
        {/* GitHub Icon */}
        <div
          className="relative flex h-10 w-10 items-center justify-center rounded-xl border"
          style={{
            borderColor: 'color-mix(in srgb, var(--border) 70%, transparent)',
            backgroundColor: 'color-mix(in srgb, var(--surface-card) 60%, transparent)',
          }}
        >
          <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" style={{ color: 'var(--text-primary)' }}>
            <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
          </svg>
          <span
            className="absolute -top-1 -right-1 h-2.5 w-2.5 rounded-full border-2"
            style={{
              backgroundColor: statusColor.bg,
              borderColor: 'var(--surface-elevated)',
            }}
          />
        </div>

        {/* Status Info */}
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
            GitHub
          </p>
          {isLoading ? (
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              Checking configuration...
            </p>
          ) : error ? (
            <p className="text-xs" style={{ color: 'var(--color-error)' }}>
              Failed to check status
            </p>
          ) : (
            <>
              <p className="text-xs" style={{ color: status?.isConfigured ? 'var(--color-success)' : 'var(--color-warning)' }}>
                {status?.isConfigured ? (status?.hasDefaultRepository ? 'Fully Configured' : 'Token Configured') : 'Not Configured'}
              </p>
              {status?.hasDefaultRepository && status.defaultOwner && status.defaultRepo && (
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  {status.defaultOwner}/{status.defaultRepo}
                </p>
              )}
              {status?.isConfigured && !status?.hasDefaultRepository && (
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  Set default repo for easier access
                </p>
              )}
            </>
          )}
        </div>
      </div>
    </div>
  );
}
