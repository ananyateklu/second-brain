...
import { useNotes, useGenerateSummaries } from '../../../features/notes/hooks/use-notes-query';
...
export function NoteSummaryBackfill() {
    ...
    const handleGenerateSummaries = async () => {
        ...
        try {
            const result = await generateSummaries.mutateAsync(noteIds);

            if (result.successCount > 0) {
                toast.success(
                    'Summaries Generated',
                    `Successfully generated ${result.successCount} summary${result.successCount !== 1 ? 'ies' : ''}.`
                );
            }
            ...
            if (result.failureCount > 0) {
                toast.warning(
                    'Some Failed',
                    `${result.failureCount} summary${result.failureCount !== 1 ? 'ies' : ''} failed to generate.`
                );
            }
            ...
            toast.error('Generation Failed', 'Failed to generate summaries. Please try again.');
        } finally {
    ...
    const handleGenerateAll = async () => {
        ...
        setIsGenerating(true);
        try {
            const result = await generateSummaries.mutateAsync(
                notesWithoutSummaries.map((n) => n.id)
            );
                ...
                {notesWithoutSummaries.length > 0 && (
                    <div className="flex gap-2">
                        <button
                            ...
                            style={{
                                backgroundColor: selectedNotes.size > 0 && !isGenerating
                                    ? 'color-mix(in srgb, var(--color-brand-600) 15%, var(--surface-card))'
                                    : 'var(--surface-elevated)',
                                borderColor: selectedNotes.size > 0 && !isGenerating
                                    ? 'var(--color-brand-600)'
                                    : 'var(--border)',
                                color: selectedNotes.size > 0 && !isGenerating
                                    ? 'var(--color-brand-600)'
                                    : 'var(--text-secondary)',
                            }}
                        >
                        ...
                        <button
                            ...
                            className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
                            style={{
