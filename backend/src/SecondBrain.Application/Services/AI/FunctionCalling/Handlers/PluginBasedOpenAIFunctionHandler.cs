...
/// <summary>
/// A plugin-based OpenAI function handler that uses reflection to expose
/// all [KernelFunction] methods from a plugin as individual OpenAI functions.
/// This mirrors how Claude, Gemini, and Ollama's native tool calling works in AgentService.
/// </summary>
public class PluginBasedOpenAIFunctionHandler : IOpenAIFunctionHandler
{
    ...
    public PluginBasedOpenAIFunctionHandler(
        IAgentPlugin plugin,
        MethodInfo method,
        ILogger logger)
    {
        ...
        _toolDefinition = OpenAIFunctionDeclarationBuilder.BuildFromMethod(method, funcAttr)
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug("Executing OpenAI function '{FunctionName}' for user '{UserId}'",
            ...
            _logger.LogDebug("OpenAI function '{FunctionName}' completed successfully", _functionName);

            return OpenAIFunctionExecutionResult.Ok(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error executing OpenAI function '{FunctionName}'", _functionName);
            return OpenAIFunctionExecutionResult.Fail($"Error: {ex.Message}");
        }
    }
    ...
    /// </summary>
    public static IEnumerable<PluginBasedOpenAIFunctionHandler> CreateFromPlugin(
        IAgentPlugin plugin,
        ILogger logger)
    {
        ...
        foreach (var method in pluginInstance.GetType().GetMethods())
        {
            ...
            if (funcAttr != null)
            {
                yield return new PluginBasedOpenAIFunctionHandler(plugin, method, logger);
            }
        }
    }
}
