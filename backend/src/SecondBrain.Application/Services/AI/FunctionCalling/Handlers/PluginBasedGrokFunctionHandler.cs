...
/// <summary>
/// A plugin-based Grok/X.AI function handler that uses reflection to expose
/// all [KernelFunction] methods from a plugin as individual Grok functions.
/// Since Grok uses an OpenAI-compatible API, this mirrors the OpenAI implementation.
/// </summary>
public class PluginBasedGrokFunctionHandler : IGrokFunctionHandler
{
    private readonly IAgentPlugin _plugin;
    private readonly MethodInfo _method;
    private readonly ChatTool _toolDefinition;
    private readonly ILogger _logger;
    private readonly string _functionName;

    public string FunctionName => _functionName;

    public PluginBasedGrokFunctionHandler(
        IAgentPlugin plugin,
        MethodInfo method,
        ILogger logger)
    {
        ...
        _toolDefinition = GrokFunctionDeclarationBuilder.BuildFromMethod(method, funcAttr)
            ?? throw new ArgumentException("Failed to build ChatTool definition");
    }

    public ChatTool GetToolDefinition() => _toolDefinition;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug("Executing Grok function '{FunctionName}' for user '{UserId}'",
            ...
            _logger.LogDebug("Grok function '{FunctionName}' completed successfully", _functionName);

            return GrokFunctionExecutionResult.Ok(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error executing Grok function '{FunctionName}'", _functionName);
            return GrokFunctionExecutionResult.Fail($"Error: {ex.Message}");
        }
    }
    ...
    /// </summary>
    public static IEnumerable<PluginBasedGrokFunctionHandler> CreateFromPlugin(
        IAgentPlugin plugin,
        ILogger logger)
    {
        ...
        foreach (var method in pluginInstance.GetType().GetMethods())
        {
            ...
            if (funcAttr != null)
            {
                yield return new PluginBasedGrokFunctionHandler(plugin, method, logger);
            }
        }
    }
}
